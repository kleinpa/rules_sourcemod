...
CORE_COPTS = select({
    # /std:c++14, not this repo's usual c++20 (sourcemod/server.bzl's
    # _COPTS_COMMON, SourceMod's own, separately-versioned AMBuildScript's
    # choice): upstream's own AMBuildScript (configure_gcc()/
    # configure_msvc(); hl2sdk-manifests carries SourceMod's copy, Metamod
    # its own inline one) targets c++14 for Metamod specifically. Metamod's
    # loader still has a `for (register size_t i = ...)` (loader/utility.cpp,
    ...
    # silences the deprecation warning -- but from ISO c++17 on `register`
    # isn't deprecated, it's removed from the grammar, a hard error
    # `-Wno-register` cannot suppress no matter how permissive the rest of
    # the warning configuration is. The mismatch went unnoticed until
    # something actually built this loader through a strict-C++17-or-later
    # toolchain (the cross-compiling clang-cl one, see
    # windows/xwin_sysroot.bzl): @rules_cc//cc/compiler:msvc-cl does match
    # that one, so an earlier attempt at this fix that only touched the
    # "//conditions:default" branch below silently never took effect.
