...
def package_providers(dest, files, deps = []):
    ...
    Args:
      ...
      deps: targets whose SourceModPackageInfo should be merged in.
    ...
    return [
        ...
        SourceModPackageInfo(
            files = depset(
                direct = [(dest, f) for f in files],
                transitive = [
                    dep[SourceModPackageInfo].files
                    for dep in deps
                    if SourceModPackageInfo in dep
                ],
            ),
        ),
    ]
