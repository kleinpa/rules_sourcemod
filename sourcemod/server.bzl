...
by the `hl2sdk` extension alongside the engine branch). sourcemod.logic.so
statically links @sourcemod_sdk's SourcePawn VM directly -- see
@sourcemod_sdk//:sourcepawn_vm's comment. This macro then assembles those
outputs together with static data files into a single pkg_filegroup -- a
PackageFilegroupInfo-bearing target,
...
def sourcemod_game_server(
    ...
    #
    # :sourcepawn_vm because core/logic/common_logic.cpp calls
    # sp::Environment::New() directly to get the SourcePawn VM -- see
    # @sourcemod_sdk//:sourcepawn_vm's comment.
    # -----------------------------------------------------------------------
    cc_library(
        name = p + "logic_lib",
        srcs = [_SM + "//:logic_srcs"],
    ...
    # Core and logic live in bin/x64/ on a 64-bit target, bin/ directly on a
    # 32-bit one. No separate SourcePawn runtime file to package -- logic_lib
    # above statically links the VM directly. The .so/.dll suffix is
    # OS-keyed the same way the loader's is above.
    pkg_files(
